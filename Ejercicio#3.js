...
//     let temperatura_maxima = readlineSync.question(
//       `Ingrese la temperatura maxima registrada del dia ${dia}: `
//     );
//     let temperatura_minima = readlineSync.question(
...
const RegistrarTemperaturas = () =>{
    ...
    let dia
    while (respuesta===1){
   let respuestaT = readlineSync.question('¿Va a registrar mas temperaturas? S/N ');
    respuestaT=='S'? respuesta=1 : respuesta=0
    let respuestaDiaria = readlineSync.question('¿Desea registrar temperaturas el dia de hoy? S/N ')
    if(respuestaDiaria!='S'){
         dia = readlineSync.question('No va a registrar temperatura el dia de hoy, para mantener un registro de dias ingrese el dia de hoy: ')
        let registroDiario ={
            dia: dia,
            no_registrado: true
        }
        ...
        dia = readlineSync.question('Ingrese el dia de hoy que va a registrar las temperaturas: ')
        respuestaDiaria=1
    }
    let ArrTemperaturas = []
    while (respuestaDiaria===1) {
    let temperatura = readlineSync.question('Ingrese la temperatura capturada: ')
    
    ArrTemperaturas.push(parseFloat(temperatura))

    let respuestaDiaria = readlineSync.question('¿Desea registrar otra temperatura el dia de hoy? S/N ')
   if(respuestaDiaria=='S'){
    respuestaDiaria=1
   }else{
